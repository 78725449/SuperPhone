...
> **配套文档**：`说明文档.md`（架构/时序/实现真相）、`AGENTS.md`（**工作区顶层索引**——规则摘要 + 各文档概览；**已知坑详情在 `docs/known-issues/`（4 份）**）、`TrollVNC/README.md`（设备端快速开始）、`trollvnc-farm/README.md`（网关启动）。
...
│  AGENTS.md            工作区顶层索引（规则摘要+文档地图）             │
│  docs/known-issues/   已知坑详情（4 份：网关协议/设备构建/数据定位/发布）│
└─────────────────────────────────────────────────────────────────────┘
```
...
├── .github/workflows/build.yml          # CI（macOS 编译 4 scheme）
├── AGENTS.md                            # 工作区顶层索引（规则摘要 + 文档地图）
├── docs/known-issues/                   # 已知坑详情（4 份活纪律）
│   ├── 01-网关与协议.md
│   ├── 02-设备端与构建.md
│   ├── 03-数据与定位.md
│   └── 04-发布与推送.md
├── docs/历史决策与排查存档.md            # 已完结的历史归档
├── 说明文档.md                          # 项目知识库（真相文档）
├── CodeWiki.md                          # 本文档
│
