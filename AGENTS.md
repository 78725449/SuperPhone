...
- **数据填充行为定稿（2026-08-25）**：① App 前端随机种子组件已移除，`generate:` 传 `seed=0` 完全随机（每次不可复现，符合随机拟真目标；5801/5802/网关 invoke 仍可传具体 seed 复现）；② 比例滑条组**真互斥**（拖动任一项其余等比缩放 + 末项补余量，合计恒 100%——旧"末项补足不对称"（拖末项不联动前面）已消除）；③ **ObjC ratios 消费归一化**（`r = rand01()*total`）对齐 Node `weightedIndex`——合计≠100 时差额**等比分配**，勿回归旧"塞首项"（contacts=friend/calls=呼入/sms=family）。改此链路时两端归一化语义必须一致。
