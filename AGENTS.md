...
- **定位编排联动事件（2026-08-24）**：参数变更感知 = manager 订阅 `prefs-changed` → `reloadFromPrefs` + `_paramsSignature` 含**轨迹文件 mtime 指纹**（新轨迹必重载、从当前位置最近点续播）；注入后 daemon（`_injectPointDict`/`_anchorTick`）发 `notify_post("com.82flex.trollvnc.locsim-update")` 供 App 即时刷新。**mobile plist 位置写回已删除**。**双域配置统一（2026-08-24，注入失效根因修复）**：配置源=**mobile 域**（App 写 mobile、`uploadTrackPoints` 也写 mobile plist 文件），daemon `_readPref` **mobile 优先、root 域仅兜底**——旧实现 root 域优先，daemon 曾写 root 残留 `SimLocationMode` 会**永久覆盖 App 的 mobile 写入**（App 开启定位→manager 仍读 root 旧值→不注入→locationd 不模拟→MKUserLocation 停在真实位置→聚焦按钮跟随真实位置）；排查"开启定位当前位置不跟随"先核对 `_readPref` 顺序与 root/mobile 双域残留。**注入必须每次完整重启 + 投递参数（2026-08-24，当前位置不跟随根因）**：`SimLocationManager injectPoint` 每次注入 `stop→clear→append→flush→start`（对齐 TrollBox/Geranium 广播行为）——**append-only（不重启）时 locationd 不把模拟位置广播给持续订阅的 client**，App/MKMapView 收不到（实测：开启定位后须打开系统地图等外部 App 发起新定位请求才"顺带"返回，否则本项目 App 位置永不更新）；投递参数 `locationDeliveryBehavior=1/locationInterval=1.0/locationDistance=0/locationRepeatBehavior=1` 每次一并设置（默认全 0 不投递）；排查特征：`sim.location.status` 返回 anchor 而 App 端 `locationManager.location` 仍是真实坐标、或"开启定位后当前位置不跟随、打开系统地图才激活"。**编辑重算（删除/重排）时 App 先写 `mode=anchor` 驻留当前位置（`holdAtCurrentPosition`），重算完成写新轨迹+itinerary 续播**——防重算期间沿已删除/重排的旧轨迹乱走 + 重载回跳；**续播最近点用 haversine**（与 App 截断同度量，平面平方近似在经度方向失真）。**停止态状态栏坐标强制绑定 locationd 真实位置**（无"保留最后模拟坐标"回退）；**停止=取消注入**：daemon off 分支调 `SimLocationManager stop`（`clearSimulatedLocations`）→ locationd 立即回归真实。**两态各由真相源驱动（2026-08-24 修复卡顿/错乱）**：模拟态刷新=locsim-update 事件唯一驱动（已删 App 1s 定时器双触发冗余）；停止态=真实 fix 经 didUpdateLocations 驱动状态栏+首次聚焦——两态随定位开关切换数据源，聚焦按钮/状态栏随"当前位置目标"（定位中=注入位置、停止=真实位置）而非死绑定 CLLocationManager 缓存；**两态=订阅切换（2026-08-24）**：开启/关闭统一为对 locationd 的订阅切换——关闭时 App `stopUpdatingLocation+startUpdatingLocation` 重新订阅触发 locationd 立即推真实 fix（GPS 定位完成前推可用 fix、精 fix 后校正，替代干等持续流；注意真实 fix 仍受 GPS 收敛时间限制，室内/弱信号偏慢属系统物理限制）；开启时 `startUpdatingLocation` 确保订阅（幂等）+ **立即 `MKUserTrackingModeFollow`**（MKMapView 仅 Follow 模式内部位置源持续活跃获取 → 水滴跟随 locationd 模拟位置，**不自绘**、数据源=locationd 精准反馈）；**闭环主动获取（2026-08-24）**：daemon 注入/停止落地均发 `locsim-update`（`off` 分支补发停止落地通知），App 收到即 `requestLocation` 主动获取（`requesting` 单飞），结果经 didUpdateLocations 更新状态栏/锚点——"注入→通知→获取"闭环；App `locationManager.desiredAccuracy` 放宽到 `kCLLocationAccuracyHundredMeters`（默认 Best≈10m 会被 locationd 扣住 horizontalAccuracy>10m 的模拟 fix 不推，daemon 注入 acc=3~15m 随机）+ 实现 `didFailWithError:` 记录定位失败（不再静默）；锚点经过判定改 O(锚点数) 距离阈值（<25m，passed 单调），不再每刷新全量扫描轨迹点（曾 O(A×P) haversine 主线程卡死，区域轨迹可达数万点）。**停止后地图立即聚焦真实位置**（重置 `hasFocusedRealOnce` + 聚焦一次，真实 fix 到达校正——无 lastRealLocation 兜底记录）。**App 启动一律停止态**：`readCurrentStatus` 不再恢复定位中，残留 `mode=anchor/itinerary` 强制写 `off`（daemon 对齐停止、locationd 恢复真实）——防"启动即自动开启模拟 / 真实位置被当模拟位置 / 恢复态 Follow 干扰搜索聚焦"，勿回归"恢复上次会话"旧行为。改这条链路时两端事件名/指纹须同步，勿回退"巡检感知 + 从头重放"旧机制。
