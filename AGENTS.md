...
| 路径 | 概览 | 地位 |
|---|---|---|
| **`定案-最终开发方向-2026-09-19.md`** | **★★★★★ 最高定案**：手机=「手+一张嘴」· 网关=「大脑+总机」· 客户端=「下命令的人」；**★ §十 执行引擎正式形态**（模型两件事 / 飞轮三次沉淀 / 三层确定性索引 / 采样协议铁律）；含**明确不做的 8 条** | **★ 与本文以下任何文档冲突，以它为准** |
...
| `实施设计-执行预测与蜂群资产-2026-09-19.md` | 实施细则：资产四层 / 执行预测回路 / 回传包 / 两类卡 / MCP 协议层 / **真机实测补章 §九（判据纪律·弹窗四级链·采样协议铁律）** | ⚠️ 「步骤级置信度+层级回退」「蜂群合并+三护栏」已被《定案》判为 F2/F3 不做 |
...
| **`docs/归档/`（17 份）** | **2026-09-21 阶段性整改归档**：09-18 调研类（宏观评估/缺口盘点/交互清单/深读/基座/MobileAgent×4/补齐原语/共同优点）已**被定案吸收**；定位/WiFi 链 3 份整改方案 + Superwrt 条目 = **定位系统已完结历史**；架构定案-手机为自主主体 = 蜂群部分被取代；08-28 框架提炼报告 | 追溯旧判断时读（引用审计：除 AGENTS 外无引用 ✓）|
...
## ★★★ 角色纪律（主干 §三 —— ★ 已按阶段整改对齐 2026-09-21）

**主干速记**：导演 / 演员 / 引擎 三角不变。**★ 但按《定案》§十 的引擎正式形态（2026-09-21 二次收口）**：
引擎的【决策点全在模型】（意图分析 · 查库裁决 · 复用/探索 · 监督纠错 · 沉淀裁决）——
"零模型"合法范围只有【命中资产后那段序列的内部步进】✗；
**机械执行与确定性验证（script.exec/screenBand/expect）模型委托给设备原语执行，自己守决策点**。
**★ 2026-09-21 已实证：子代理可扮演【引擎本体】端到端首跑全链路通 ✅**（视觉+工具+资产一站式）——
此前"DSH 模型逐步决定点哪里=角色错位"的表述中，错位的是【让模型盲猜坐标】✗，不是"模型在场"✗。
...
- **★★★★ OmniParser（视觉→结构化元素）判断：2026-09-19 真机实测后【已修正】** —— 原判"基本用不到"✗ 被实测推翻：① OCR 只覆盖文字类，**图标类它完全给不了**（实测 462 元素中 200 个是纯图标 = 43%）② "找模板"需要模板库，而模板库缺的正是**裁剪依据**（只有坐标点不知道裁哪里）③ dsh-ios 那条纪律被实测否掉：**覆盖率 100%（29/29 已知可点击元素全被覆盖）· 与我的 OpenCV 轮廓法独立吻合 IoU 0.539 · 近方形（图标型）占 73%/55%**。**★ 新定位：离线资产解析器**（不是实时感知层 —— 实测 1.1~1.5s/帧，离线可接受且可反复重跑历史截图）。**★ L0 图标库建法：icon_detect 取 bbox → 取纯图标元素 → pHash 聚类做骨架 → 每类取 caption 众数当名字（类内一致率 0.86，全局才 1.3%）→ 低 purity 走人工**。**★★ 采样协议铁律：`stability` 的语义由采样协议决定**（连拍测"瞬时稳定"31/32 ✗ vs 跨内容采测"结构稳定"11/60 ✓，差 3 倍）—— **二期判"抖音页面签名不可靠"，根因是采样方式而非 OCR 抖动**。详见《实施设计》§7.5.1/§7.5.2。仍值得吸收：Set-of-Mark 编号 · "UI→结构化元素表"形态。
...
- **★★ 硬前提现状（★ 2026-09-21 已更新）**：~~当前模型不支持图像输入~~ → **用户已切换为支持图像输入的模型**（本会话实证：`read_image` 可读设备截图，76×54 裁片都能看清文字 ✓）—— **"演员没有眼睛"不再是阻碍**；引擎首跑（§十）即以此为前提之一。**★ 但公理一「少走视觉」仍是性能路线**：运行期判定仍以确定性原语（OCR/screenBand/模板）优先，视觉用于决策点与抽查；架构总纲 §5"GUI 工作模型是伪需求"的结论在【运行期每步不靠模型】这一点上仍然成立 ✓。★ 空档更新：`vision.find_text` ✅ 可用；`vision.find_image` ⚠️ 素材已有（859 裁片待质检入库）→ 前置条件从"缺模板"变为"缺【筛选入库】"。
...
- `scripts/` — **工具脚本（35 个活跃）**：推送/取包（见 `docs/known-issues/04-发布与推送.md`）· **资产库读写唯一入口 `asset-store.py`** · 引擎机械附肢 `engine-prims.py` · 解析管线 `omniparser-*`/`build-*` · 审计 `audit-redundancy.py`。**★ `scripts/archive/`（50 个）= 已归档的一次性探针**（2026-09-21 冗余清理；保留可追溯性，不再维护）
