...
- **清空联系人报「通信错误」根因 + 根治（2026-08-25 真机定稿）**：联系人走 **CNContactStore XPC（依赖 contactsd 存活）**，通话/短信是 **sqlite 直写（不依赖 daemon）**。**根因是自伤**：生成/清空联系人后 `kill contactsd`（data.test 直写 DB 时代遗留）→ 打断所有 CNContactStore 客户端连接、制造重启窗口 → 紧接操作遇 `CNErrorCodeCommunicationError(1)`「通信错误」；通话/短信不依赖 daemon 故总能成功。**根治：contacts 链路（fill+clear）不再 kill contactsd**——CNContactStore 写删由 contactsd 自己执行并广播 `CNContactStoreDidChangeNotification`，系统通讯录自动刷新。**另加「无响应」防护（2026-08-25 二次修复）**：contactsd 被反复 kill 后可能进入半死状态（XPC 连上但无响应，同步调用挂起 30s+ 致 UI 无反馈）→ `trContactsRun` 看门狗（CNContactStore 操作放独立队列 + 轮询超时 30s 放弃），超时即 kill contactsd 重启自修复并反馈「服务无响应已重启，请重试」。**规则：CNContactStore 写删不 kill（仅看门狗超时自修复时 kill）；sqlite 直写（calls/sms）才 kill 对应 daemon**。另有要求：`deleteContact:` 必须显式请求 `CNContactIdentifierKey`；勿回归"直写 AddressBook.sqlitedb"（缺 FTS/触发器系统列表不显示，见说明文档 §4.8）。
