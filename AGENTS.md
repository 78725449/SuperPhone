...
- **清空联系人报「通信错误」根因（2026-08-25 真机定稿）**：联系人走 **CNContactStore XPC（依赖 contactsd 存活）**，通话/短信是 **sqlite 直写（不依赖 daemon）**——生成联系人后 `kill contactsd`（写后刷新）的重启窗口内紧接清空 → XPC 连不上 → `CNErrorCodeCommunicationError(1)`「通信错误」，多批全失败；通话/短信因不依赖 daemon 故总能成功。**修复：CNContactStore 枚举与删除批次各加 3 次重试（间隔 1s，等待 contactsd 就绪自愈）**；另有要求：`deleteContact:` 必须显式请求 `CNContactIdentifierKey`。改清空链路时勿回归"直写 AddressBook.sqlitedb"（缺 FTS/触发器系统列表不显示，见说明文档 §4.8）。
