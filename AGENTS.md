...
⑧ **执行结果 UI 定稿（2026-08-25）**：生成/清空结果弹**居中模态对话框**（`showResultDialogWithIcon:color:title:detail:`，成功=绿✓/失败=红✕/清空未完全=橙!），替代底部 resultLabel（仅留"生成中…"瞬时态）；**去"合计 组x xx%"标签**（互斥恒 100% 无需展示，重置按钮全宽独立一行）；返回契约规范化（生成 `{ok,count}`/清空 `{ok,cleared}`/部分失败 `{ok,errors}`/残留 `{ok:NO,remaining}`，信息级 `kill*`、诊断 `*Meta` 字段分层）——App 消费分层，5801/网关走原始返回。生成提示文案=「已生成 {联系人/通话记录/短信} N 条」（无"刷新生效"）。
