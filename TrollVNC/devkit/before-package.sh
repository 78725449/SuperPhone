...
# 注：Theos LIBRARY 产物不进入 staging /usr/lib（装到 Theos 自身 lib 目录），dylib 从 .theos/obj 定位
...
ND_DYLIB=$(find .theos/obj -name "netdisguise.dylib" 2>/dev/null | head -1)
if [ -z "$ND_DYLIB" ]; then echo "netdisguise.dylib not found in .theos/obj"; exit 1; fi
cp -rp "$ND_DYLIB" "$THEOS_STAGING_DIR/Applications/TrollVNC.app/"
