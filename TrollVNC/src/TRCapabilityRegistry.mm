...
// 自签证书生成已抽至共享模块 TRSelfSignedCert.mm（2026-08-19，含 IP SAN），
// settings.generateKeys 直接调用 TRGenerateSelfSignedCert（见 _registerSettingsActions）。
...
- (void)_registerSettingsActions {
    ...
    // 复用共享模块 TRSelfSignedCert.mm 的 TRGenerateSelfSignedCert，等价 ZTSelfSignedCertificate generateWithCommonName）
